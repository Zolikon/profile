...
function ProjectCard({ name, description, image }) {
  ...
  return (
    <NavLink to={name.toLowerCase()} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <motion.div
        className="relative w-[240px] h-[400px] bg-teal-400 dark:bg-teal-700 rounded-2xl border-stone-500 border-2"
      ...
      >
        {/* Front of the Card */}
        <motion.div
          className="absolute w-full h-full flex flex-col justify-center gap-2   p-2  "
